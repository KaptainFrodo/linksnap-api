...
// Health check
app.get("/", (req, res) => {
  res.json({ status: "ok", service: "LinkSnap API", version: "1.0.0" });
});

// Main decode endpoint
app.post("/decode", upload.single("image"), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({ error: "No image uploaded. Send as multipart/form-data with field name 'image'." });
  }
...
async function decodeQR(imageBuffer) {
  ...
  // Helper: get raw RGBA pixels from a sharp pipeline
  ...
  // Helper: run zbar on RGBA buffer
  ...
  // Helper: pixel transform on a raw RGBA buffer
  ...
  const stripColor = ({data, width, height}, t) => ({
    data: transform(data, (r,g,b,a) => {
      const m = (r+g+b)/3;
      const s = Math.sqrt(((r-m)**2 + (g-m)**2 + (b-m)**2)/3);
